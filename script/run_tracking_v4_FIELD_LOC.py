...
OUTPUT_PATH = "/Users/kunwoomac/CodeSpace/24-768-Team-Project/output/tracking_localization_v8_AUTO_SCALE.mp4"
...
# We no longer hardcode sensitivities. We calculate them based on zoom.
# This is the "Base" sensitivity at 1.0x Zoom.
BASE_ROTATION_SENSITIVITY = 0.00025
BASE_TILT_SENSITIVITY = 0.0
BASE_FOV_ANGLE = np.deg2rad(30) 

# *** THE REFERENCE CALIBRATION ***
# "How tall (in pixels) is a player when the camera is at 'normal' 1.0x zoom?"
# In 1080p footage, a far-side player is usually ~60px, near-side ~100px. Average ~80px.
...
class OfflineTracker:
    ...
    def pass_2_render(self):
        ...
        # Zoom now starts at 1.0 but adapts
        ...
        with tqdm(total=total_frames, desc="4/4 Rendering", unit="frame") as pbar:
            while True:
                ...
                # --- 1. DETECT DYNAMIC ZOOM (THE FEATURE SIZE LOGIC) ---
                ...
                for t_id, bbox in current_tracks:
                    # Height is y2 - y1
                    h = bbox[3] - bbox[1]
                    # Filter: ignore noise (too small) or glitches (too big)
                    if 20 < h < 400:
                ...
                if len(detected_player_heights) > 2:
                    ...
                    # Infer scale: If median height is 160px, and Ref is 80px, Zoom is 2.0x
                    target_zoom = median_h / REFERENCE_PLAYER_HEIGHT_PX
                
                # Smooth the zoom (don't jump frame to frame)
                current_zoom_level = current_zoom_level * 0.90 + target_zoom * 0.10
                
                # --- 2. ADJUST SENSITIVITY BASED ON ZOOM ---
                # If zoomed in (zoom > 1), one pixel of movement means LESS rotation degrees.
                # So we scale sensitivity DOWN by zoom level.
                # Wait: If Zoomed IN, 100 pixels is a tiny move.
                # If Zoomed OUT, 100 pixels is a huge move.
                # Actually, standard formula: Angle = atan(pixel / focal_length). 
                # Zooming increases focal length. So Angle per pixel decreases.
                        ...
                        if len(good_new) > 4:
                            ...
                            if H is not None and mask is not None and np.sum(mask) > 4:
                                ...
                                if m is not None:
                                    ...
                                    # We IGNORE d_zoom from optical flow now, relying on Player Height
                ...
                # --- 4. PERSPECTIVE (USING DYNAMIC ZOOM) ---
                # Zoom in -> FOV gets smaller
                ...
                # Draw Telemetry
                cv2.putText(current_minimap, f"Zoom: {current_zoom_level:.2f}x", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)
