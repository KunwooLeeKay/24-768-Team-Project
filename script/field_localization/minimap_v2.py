...
def main():
    ...
    while True:
        ...
        # Adjusted to be much wider to match the video's actual zoom
        fov_width_far = 900   # Was 600
        fov_width_near = 450  # Was 250
        ...
        # --- VIDEO OVERLAY LOGIC ---
        # 1. Define Source Points (Video Frame Corners)
        # CRITICAL FIX: We crop the top 20% of the video source (stands/scoreboard)
        # so we only map the actual GRASS to the minimap. This fixes vertical scaling/squashing.
        crop_top_percent = 0.20 
        src_top_y = int(vid_h * crop_top_percent)
        
        src_pts = np.float32([
            [0, src_top_y],       # Top-Left (below stands)
            [vid_w, src_top_y],   # Top-Right (below stands)
            [vid_w, vid_h],       # Bottom-Right
            [0, vid_h]            # Bottom-Left
        ])
        
        # 2. Define Destination Points (FOV Trapezoid Corners)
        ...
        # 4. Warp the actual video frame
        # Note: warpPerspective will fill the outside with black (0), which is fine for masking
        warped_frame = cv2.warpPerspective(frame, M_persp, (minimap_w, minimap_h))
        
        # 5. Create a mask for the FOV area
        mask = np.zeros((minimap_h, minimap_w), dtype=np.uint8)
        cv2.fillPoly(mask, [fov_poly], 255)
        
        # 6. Blend the warped video onto the minimap with transparency
        alpha_overlay = 0.4
