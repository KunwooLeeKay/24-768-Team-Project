...
output_path = "/Users/kunwoomac/CodeSpace/24-768-Team-Project/output/YOLO_v4.mp4"
...
track_trail = defaultdict(lambda: deque(maxlen=30))  # Store center points for trails

# Team classification setup
track_teams = {}
track_teams_locked = {}  # Once assigned, never change for trail consistency
team_colors_display = {1: (0, 255, 0), 2: (255, 255, 255)}
team_vote_history = defaultdict(lambda: deque(maxlen=15))
team_color_scores = defaultdict(lambda: deque(maxlen=10))
...
def classify_uniform_by_clustering(frame, bbox, debug=False):
    ...
    dominant_clusters = np.argsort(cluster_sizes)[::-1]
...
SHOW_TRAILS = True  # Show movement trails for players
...
while True:
    ...
    if results.boxes is not None and results.boxes.id is not None:
        ...
        for box, track_id, conf, cls in zip(boxes, track_ids, confidences, classes):
            ...
            if cls == 0:  # Person
                ...
                center_x = int((smoothed_box[0] + smoothed_box[2]) / 2)
                center_y = int((smoothed_box[1] + smoothed_box[3]) / 2)
                track_trail[track_id].append((center_x, center_y))
                ...
                if team is not None:
                    if track_id not in track_teams:
                        ...
                        track_teams_locked[track_id] = team  # Lock permanently for trails
                    # If already assigned, only change if overwhelming evidence (90%+ opposite votes)
                        ...
                        if len(votes) >= 10:
                            ...
                            if opposite_votes >= 9:  # 90%+ evidence to switch
                                track_teams[track_id] = team
                                # But DON'T change the locked team for trail consistency
                                if track_id not in track_teams_locked:
                                    track_teams_locked[track_id] = team
                ...
                if SHOW_TRAILS and len(track_trail[track_id]) > 1:
                    trail_points = list(track_trail[track_id])
                    
                    # Use LOCKED team assignment for consistent trail color
                    # This ensures the trail never changes color for the same tracked entity
                    if track_id in track_teams_locked:
                        team_for_trail = track_teams_locked[track_id]
                        if team_for_trail == 1:
                            trail_color = (0, 200, 0)  # Green for team 1
                        else:
                            trail_color = (200, 200, 200)  # White/gray for team 2
                    else:
                        trail_color = (100, 100, 100)  # Gray for unclassified
                    
                    # Draw lines with progressive thickness
                    for i in range(1, len(trail_points)):
                        # Calculate thickness based on recency (newer = thicker)
                        thickness = max(1, int(2 * i / len(trail_points)))
                        cv2.line(frame, trail_points[i-1], trail_points[i], trail_color, thickness)
                    
                    # Draw a circle at current position
                    cv2.circle(frame, trail_points[-1], 3, trail_color, -1)
    ...
    # Draw legend
    legend_y = 30
    legend_height = 110 if SHOW_TRAILS else 90
    cv2.rectangle(frame, (10, 10), (250, legend_height), (0, 0, 0), -1)
    ...
    if SHOW_TRAILS:
        cv2.putText(frame, "Trails: ON", (20, legend_y + 75), 
