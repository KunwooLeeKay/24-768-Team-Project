...
OUTPUT_PATH = "/Users/kunwoomac/CodeSpace/24-768-Team-Project/output/tracking_localization_v10_AUTO_VERTICAL.mp4"
...
BASE_ROTATION_SENSITIVITY = 0.0003 
BASE_TILT_SENSITIVITY = 0.01
BASE_FOV_ANGLE = np.deg2rad(40) 
...
MINIMAP_WIDTH = 600   
MINIMAP_HEIGHT = 400
PIVOT_OFFSET_Y = 600  # Back to original offset
...
# Visuals
COLOR_TEAM_1 = (0, 200, 0)      
COLOR_TEAM_2 = (220, 220, 220)  
COLOR_BALL_DOT = (0, 0, 255)    
COLOR_FOV_LINES = (50, 50, 50)  
...
class OfflineTracker:
    ...
    def create_soccer_field(self, width, height):
        field_color = (40, 45, 40); line_color = (200, 200, 200); line_thickness = 2
    ...
    # --- STEP 3: RENDERING (AUTO VERTICAL ADJUSTMENT) ---
    def pass_2_render(self):
        ...
        smooth_cam_angle, smooth_cam_tilt = 0.0, 0.0
        current_zoom_level = 1.0
        
        # Smoothed vertical offset
        ...
        with tqdm(total=total_frames, desc="4/4 Rendering", unit="frame") as pbar:
            while True:
                ...
                # --- 1. DETECT DYNAMIC ZOOM ---
                detected_player_heights = []
                current_tracks = frame_map[frame_idx]
                
                for t_id, bbox in current_tracks:
                    h = bbox[3] - bbox[1]
                    if 20 < h < 400:
                        detected_player_heights.append(h)
                
                target_zoom = 1.0
                if len(detected_player_heights) > 2:
                    median_h = np.median(detected_player_heights)
                    target_zoom = median_h / REFERENCE_PLAYER_HEIGHT_PX
                
                current_zoom_level = current_zoom_level * 0.90 + target_zoom * 0.10
                
                # --- 2. ADJUST SENSITIVITY ---
                effective_rot_sens = BASE_ROTATION_SENSITIVITY / current_zoom_level
                effective_tilt_sens = BASE_TILT_SENSITIVITY / current_zoom_level
                    ...
                    if p1 is not None:
                        ...
                        if len(good_new) > 4:
                            ...
                            if H is not None and mask is not None and np.sum(mask) > 4:
                                ...
                                if m is not None:
                                    raw_da = m[0, 2] * effective_rot_sens
                                    raw_dt = -m[1, 2] * effective_tilt_sens
                                    
                                    if abs(raw_da) > 0.00001: d_angle = raw_da
                ...
                cam_angle += d_angle; cam_tilt += d_tilt
                cam_tilt = max(-100, min(cam_tilt, 100))
                
                smooth_cam_angle = smooth_cam_angle * 0.90 + cam_angle * 0.10
                smooth_cam_tilt = smooth_cam_tilt * 0.90 + cam_tilt * 0.10

                # --- 4. PERSPECTIVE MAPPING (ORIGINAL METHOD) ---
                current_fov = BASE_FOV_ANGLE / current_zoom_level
                
                y_far = max(0, safe_int(40 + smooth_cam_tilt))
                y_near = min(minimap_h, safe_int(minimap_h - 40 + smooth_cam_tilt))
                
                theta_left = smooth_cam_angle + (current_fov / 2)
                theta_right = smooth_cam_angle - (current_fov / 2)
                ...
                src_pts = np.float32([[0, safe_int(vid_h * FEATURE_CROP_TOP_PERCENT)], 
                                      [vid_w, safe_int(vid_h * FEATURE_CROP_TOP_PERCENT)], 
                                      [vid_w, vid_h], [0, vid_h]])
                dst_pts = np.float32([(x_far_left, y_far), (x_far_right, y_far), 
                                      (x_near_right, y_near), (x_near_left, y_near)])
                ...
                if len(points_to_transform) > 0:
                    ...
                    transformed_points = [(safe_int(pt[0]), safe_int(pt[1]), colors_for_points[i]) 
                                         for i, pt in enumerate(transformed)]

                # --- 6. AUTO VERTICAL ADJUSTMENT ---
                # Calculate how far players are from field boundaries
                field_margin = 30
                field_top = field_margin
                field_bottom = minimap_h - field_margin
                
                target_offset = 0
                if len(transformed_points) > 0:
                    y_positions = [pt[1] for pt in transformed_points]
                    min_y = min(y_positions)
                    max_y = max(y_positions)
                    
                    # If players are above field, shift down
                    if min_y < field_top:
                        target_offset = field_top - min_y
                    # If players are below field, shift up
                    elif max_y > field_bottom:
                        target_offset = field_bottom - max_y
                
                # Smooth the adjustment
                smooth_vertical_offset = smooth_vertical_offset * 0.85 + target_offset * 0.15
                
                # --- 7. DRAW MAP ---
                current_minimap = base_field.copy()
                
                fov_poly = np.array([(x_near_left, y_near), (x_far_left, y_far), 
                                     (x_far_right, y_far), (x_near_right, y_near)], np.int32)
                ...
                cv2.putText(current_minimap, f"Zoom: {current_zoom_level:.2f}x", 
                           (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)
                cv2.putText(current_minimap, f"V-Offset: {smooth_vertical_offset:.1f}", 
                           (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)

                # Draw points with vertical adjustment
                for mx, my, color in transformed_points:
                    adjusted_y = safe_int(my + smooth_vertical_offset)
                    if 0 <= mx < minimap_w and 0 <= adjusted_y < minimap_h:
                        cv2.circle(current_minimap, (mx, adjusted_y), 5, color, -1)
