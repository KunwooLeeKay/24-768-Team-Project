...
OUTPUT_PATH = "/Users/kunwoomac/CodeSpace/24-768-Team-Project/output/tracking_dual_model.mp4"

# *** MODEL SETTINGS ***
PLAYER_MODEL = "yolov8x.pt"       # Standard model for people
BALL_MODEL_PATH = "ball_detection_model.pt" # <--- YOUR CUSTOM MODEL HERE (Fix 3)
...
class OfflineTracker:
    ...
    def pass_1_analysis(self):
        ...
        # FIX 1: CALCULATE HIGH RES
        # Ensure we run inference at high resolution for the ball
        ball_inference_size = max(1280, vid_width)
        ball_inference_size = (ball_inference_size // 32) * 32 # Multiple of 32
        ...
        with tqdm(total=total_frames, desc="1/4 Analysis (Dual Pass)", unit="frame") as pbar:
            while True:
                ...
                # --- PASS B: BALL (Custom Model + High Res) ---
                # Fix 1: High Res (imgsz)
                # Fix 3: Custom Model
                # Note: In custom single-class models, the ball is usually Class 0
                b_results = self.model_ball.track(frame, persist=True, verbose=False, imgsz=ball_inference_size, conf=0.5)
                
                ball_found = False
                if b_results[0].boxes.id is not None:
                    # Get all detections
                    boxes_xywh = b_results[0].boxes.xywh.cpu().numpy()
                    confidences = b_results[0].boxes.conf.cpu().numpy()
                    
                    # Find highest confidence detection
                    best_idx = np.argmax(confidences)
                    x, y, w, h = boxes_xywh[best_idx]
                    
                    # Basic sanity check: Ball shouldn't be the size of the whole screen
                    if w < vid_width * 0.1:
                        self.ball_raw.append({'frame': frame_idx, 'x': x, 'y': y, 'w': w, 'h': h})
                        ball_found = True
